...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ApiError> accessDenied(AccessDeniedException exception, HttpServletRequest request) {
        return response(HttpStatus.FORBIDDEN, "Forbidden", "You do not have permission to perform this action", request, Map.of());
    }
    ...
    private ResponseEntity<ApiError> response(HttpStatus status, String error, String message,
        ...
        return ResponseEntity.status(Objects.requireNonNull(status)).body(new ApiError(Instant.now(), status.value(), error,
