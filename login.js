  ...
  form.addEventListener('submit', async event => {
    ...
    if (window.HlokomelaAPI) {
      try {
        ...
        /* If error looks like a network/server error, fall through to demo mode */
        const msg = loginError.message || '';
        const isNetworkError = msg.includes('Failed to fetch') || msg.includes('NetworkError') || msg.includes('Request failed (5') || msg.includes('Load failed');
        if (!isNetworkError) {
